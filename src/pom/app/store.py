...
class Store(BasePage):
    ...
    class loc:
        ...
        upgrade_all = Selector(XPath(has_text="Upgrade All"), description="升级所有")
        pause = Selector(XPath(has_text="Pause"), description="暂停按钮")
        open = Selector(XPath(has_text="Open"), description="打开")
        my_profile = Selector(XPath(has_text="My Profile"), description="我的")
        update_list = Selector(XPath(has_text="Application Update"), description="更新列表")
        inst_list = Selector(XPath(has_text="Installation List"), description="安装列表")
        chrome = Selector(XPath(has_text="Chrome"), description="chrome浏览器")
        about = Selector(XPath(has_text="About"), description="关于")
        no_down = Selector(XPath(has_text="No download tasks"), description="无下载队列")
        no_update = Selector(XPath(has_text="The current application is the latest version"), description="无更新应用")
    ...
    def get_no_update(self):
        console.print("等待更新完所有应用，最多等待30分钟")
        return self.driver.find(self.loc.no_update, exist_ok=True, timeout=1800)
    ...
    def get_down(self):
        return self.driver.find(self.loc.no_down, timeout=300, exist_ok=True)
